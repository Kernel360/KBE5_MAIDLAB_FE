    ...
    <>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          {
            key: 'APPROVED',
            ...getStatusInfo('APPROVED'),
            count: statusStats.APPROVED.count,
            amount: statusStats.APPROVED.amount,
          },
          {
            key: 'PENDING',
            ...getStatusInfo('PENDING'),
            count: statusStats.PENDING.count,
            amount: statusStats.PENDING.amount,
          },
          {
            key: 'REJECTED',
            ...getStatusInfo('REJECTED'),
            count: statusStats.REJECTED.count,
            amount: statusStats.REJECTED.amount,
          },
        ].map((status) => {
          ...
          return (
            <div
              key={status.key}
              className={`${status.bg} rounded-2xl p-4 border ${status.border}`}
            >
              <div
                className={`w-10 h-10 rounded-xl bg-gradient-to-br ${status.gradient} flex items-center justify-center mb-3`}
              >
