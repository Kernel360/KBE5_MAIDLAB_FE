...
const SettlementCard: React.FC<SettlementCardProps> = ({ settlement, onViewDetails }) => {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm hover:shadow-md transition-all duration-200">
      <div className="flex items-center gap-4 mb-4">
        <div className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${serviceInfo.gradient} flex items-center justify-center`}>
        ...
        <div className="flex items-center gap-2">
          <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium ${statusInfo.bg} ${statusInfo.text} border ${statusInfo.border}`}>
      ...
      <div className="flex items-end justify-between pt-3 border-t border-gray-100">
        ...
        <div className="text-right">
          ...
          <div className={`text-2xl font-bold ${
            settlement.status === 'APPROVED' ? 'text-emerald-600' :
            settlement.status === 'PENDING' ? 'text-amber-600' :
            settlement.status === 'REJECTED' ? 'text-red-600 line-through' :
            'text-gray-900'
          }`}>
