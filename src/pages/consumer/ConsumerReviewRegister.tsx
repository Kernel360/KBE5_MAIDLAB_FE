    ...
    <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              ...
              {group.keywords.map((keyword) => {
                ...
                return (
                  <button
                    ...
                    disabled={isDisabled}
                    className={`px-3 py-1.5 rounded-full text-sm transition-all ${isSelected ? 'bg-blue-100 text-blue-700 border-2 border-blue-300' : isDisabled ? 'bg-gray-50 text-gray-400 border-2 border-transparent cursor-not-allowed' : 'bg-gray-100 text-gray-600 border-2 border-transparent hover:bg-gray-200'}`}
                  >
                    {keyword}
                  </button>
                );
      ...
      {selectedKeywords.length > 0 && (
        <div className="mt-6 p-4 bg-blue-50 rounded-xl">
          <h4 className="text-sm font-medium text-blue-800 mb-2">선택된 키워드 ({selectedKeywords.length}/{maxKeywords}개)</h4>
          <div className="flex flex-wrap gap-2">
            {selectedKeywords.map((keyword) => (
              <div key={keyword} className="relative inline-flex items-center px-3 py-1.5 bg-blue-200 text-blue-800 text-xs rounded-full pr-8">
                <span>{keyword}</span>
                <button type="button" onClick={() => onKeywordRemove(keyword)} className="absolute right-1 top-1/2 -translate-y-1/2 w-4 h-4 bg-blue-300 hover:bg-blue-400 rounded-full flex items-center justify-center transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1">
...
const ReviewTextArea: React.FC<{
  ...
  const isValid = comment.length >= LENGTH_LIMITS.REVIEW_COMMENT.MIN;
...
const ConsumerReviewRegister: React.FC = () => {
  ...
  const isCommentValid = formData.comment.length >= LENGTH_LIMITS.REVIEW_COMMENT.MIN && formData.comment.length <= LENGTH_LIMITS.REVIEW_COMMENT.MAX;
  const isFormValid = isKeywordValid || isCommentValid;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isFormValid) {
      alert(`리뷰는 키워드 1개 이상 선택 또는 ${LENGTH_LIMITS.REVIEW_COMMENT.MIN}자 이상 ${LENGTH_LIMITS.REVIEW_COMMENT.MAX}자 이하로 작성해주세요.`);
      return;
    }
