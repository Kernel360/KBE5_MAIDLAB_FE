  ...
  const categorizeActivity = (logLine: string) => {
    ...
    if (classMatch) {
      ...
      if (className.includes('consumer')) {
        ...
        className.includes('reservation') ||
        className.includes('booking')
      ) {
        return '예약';
      } else if (
        className.includes('payment') ||
        className.includes('settlement')
      ) {
        return '정산';
  ...
  const connectWebSocket = () => {
    try {
      const newSocket = new WebSocket(
        'wss://api-maidlab.duckdns.org/admin/logs/stream',
      );
      ...
      newSocket.onmessage = (event) => {
        try {
          ...
          if (Array.isArray(data)) {
            ...
            logContent = Array.isArray(data.logs)
              ? data.logs.join('\n')
              : data.logs;
          } else {
          ...
          if (serviceLogs.length > 0) {
            setRecentLogs((prev) => {
              ...
              const uniqueLogs = newLogs.filter(
                (newLog) =>
                  !prev.some(
                    (existingLog) => existingLog.trim() === newLog.trim(),
                  ),
              );
          ...
          ? // Show loading skeleton cards
            Array.from({ length: 8 }).map((_, index) => (
              <div key={index} className="bg-white rounded-lg shadow-md p-6">
                <div className="text-sm text-gray-300 mb-2 bg-gray-200 h-4 rounded animate-pulse"></div>
                <div className="text-2xl font-semibold bg-gray-200 h-8 rounded animate-pulse"></div>
              </div>
            ))
          : stats.map((stat) => (
              <div
          ...
          <div className="p-6">
            {logsLoading ? (
              ...
              <div className="text-center py-8">
                <span className="text-sm text-red-600">{logsError}</span>
              </div>
              ...
              recentLogs
                ...
                .map((parsedLog, idx) => {
                  const formattedTime = parsedLog.timestamp.toLocaleString(
                    'ko-KR',
                    {
                      month: '2-digit',
                      day: '2-digit',
