...
const ManagerDetail = () => {
  ...
  const [managerData, setManagerData] = useState<AdminManagerDetail | null>(null);
  ...
  useEffect(() => {
    const fetchManagerData = async () => {
      try {
        ...
        // 총 매칭 건수 조회
        const matchedCount = await (adminApi as any).getMatchedCount(parseInt(id));
        setTotalReservations(matchedCount);
        
        // 총 수익 금액 조회
        const settlementSum = await (adminApi as any).getManagerSettlementSum(parseInt(id));
        setTotalEarnings(settlementSum);
        
        // 리뷰 작성률 조회 (temporarily using type assertion until TS recognizes the function)
        const reviewPercent = await (adminApi as any).getManagerReviewPercent(parseInt(id));
  ...
  const getRecentReservations = async () => {
    ...
    try {
      ...
      const reservations = await reservationManagement.fetchManagerReservation(parseInt(id), params);
    ...
    return (
      <div className="max-w-5xl mx-auto p-6">
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
          <p className="text-yellow-800 font-medium">매니저를 찾을 수 없습니다.</p>
        </div>
        <button
          className="mt-4 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700"
          onClick={() => navigate(-1)}
    ...
    <div className="max-w-5xl mx-auto p-6">
        ...
        <div className="bg-yellow-100 border-l-4 border-yellow-500 p-4 mb-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              ...
              <p className="text-yellow-800 font-medium">⏳ 승인 대기 중인 매니저입니다</p>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-green-50 rounded-lg p-4 border border-green-200">
          <div className="text-green-800 font-semibold text-sm">총 매칭 건수</div>
          <div className="text-2xl font-bold text-green-900">{totalReservations}건</div>
        </div>
        <div className="bg-purple-50 rounded-lg p-4 border border-purple-200">
          <div className="text-purple-800 font-semibold text-sm">총 수익 금액</div>
          <div className="text-2xl font-bold text-purple-900">
            {totalEarnings.toLocaleString()}원
          </div>
        </div>
        <div className="bg-orange-50 rounded-lg p-4 border border-orange-200">
          <div className="text-orange-800 font-semibold text-sm">리뷰 작성률</div>
          <div className="text-2xl font-bold text-orange-900">{matchingRate}%</div>
        </div>
      </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <div className="flex items-center space-x-4 mb-6">
              ...
              <div>
                <div className="flex items-center space-x-2 mb-1">
                  <h2 className="text-xl font-bold text-gray-900">{managerData.name}</h2>
                  <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${
                    managerData.isVerified === 'APPROVED' ? 'bg-green-100 text-green-800' :
                    managerData.isVerified === 'PENDING' ? 'bg-yellow-100 text-yellow-800' :
            ...
            <div className="space-y-3">
              <div className="flex justify-between">
                ...
                <span className="text-gray-900">{managerData.gender === 'MALE' ? '남성' : '여성'}</span>
              </div>
              <div className="flex justify-between">
                ...
                <span className="text-gray-900">
                  {managerData.socialType ? (
                    <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                      {managerData.socialType === 'GOOGLE' ? '구글' : managerData.socialType}
                    </span>
                  ) : (
                    '일반 계정'
                  )}
              ...
              <div className="flex justify-between">
                ...
                <span className="text-gray-900">{formatDate(managerData.createdAt)}</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            {recentReservations.length > 0 ? (
                  ...
                  >
                    ...
                    <div className="mb-2">
                      <h4 className="font-semibold text-gray-900 text-base">
                        {getServiceTypeName(reservation.serviceType)} - {reservation.detailServiceType}
                      </h4>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        ...
                        <div className="text-gray-900 font-medium">{reservation.reservationDate}</div>
                      </div>
                      <div>
                        <span className="text-gray-500 font-medium">서비스 시간:</span>
              ...
              <div className="bg-gray-50 rounded-lg p-12 text-center">
                ...
                <p className="text-gray-500 font-medium text-lg">매칭 기록이 없습니다</p>
                <p className="text-gray-400 text-sm mt-2">아직 매칭된 서비스가 없습니다</p>
              </div>
            )}
          </div>
        </div>
