...
import type { ConsumerProfileDetail } from '@/types/admin';
...
const ConsumerDetail = () => {
  ...
  useEffect(() => {
    const fetchConsumerData = async () => {
      try {
        ...
        // 총 예약 건수 조회
        const totalCount = await adminApi.getTotalReservationCount(parseInt(id));
  ...
  const getRecentReservations = async () => {
    ...
    const reservations = await reservationManagement.fetchConsumerReservation(parseInt(id), params);
    ...
    return (
      <div className="max-w-5xl mx-auto p-6">
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
          <p className="text-yellow-800 font-medium">소비자를 찾을 수 없습니다.</p>
        </div>
        <button
          className="mt-4 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700"
          onClick={() => navigate(-1)}
    ...
    <div className="max-w-5xl mx-auto p-6 bg-gray-50">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-green-50 rounded-lg p-4 border border-green-200">
          <div className="text-green-800 font-semibold text-sm">총 예약 건수</div>
          <div className="text-2xl font-bold text-green-900">{totalReservations}건</div>
        </div>
        <div className="bg-purple-50 rounded-lg p-4 border border-purple-200">
          <div className="text-purple-800 font-semibold text-sm">총 결제 금액</div>
        ...
        <div className="bg-orange-50 rounded-lg p-4 border border-orange-200">
          ...
          <div className="text-2xl font-bold text-orange-900">{reviewPercent}%</div>
        </div>
      </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <div className="flex items-center space-x-4 mb-6">
              ...
              <div>
                <div className="flex items-center space-x-2 mb-1">
                  <h2 className="text-xl font-bold text-gray-900">{consumerData.name}</h2>
                  {consumerData.isDeleted && (
            ...
            <div className="space-y-3">
              <div className="flex justify-between">
                ...
                <span className="text-gray-900">{consumerData.gender === 'MALE' ? '남성' : '여성'}</span>
              </div>
              <div className="flex justify-between">
                ...
                <span className="text-gray-900">
                  {consumerData.socialType ? (
                    <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                      {consumerData.socialType === 'GOOGLE' ? '구글' : consumerData.socialType}
                    </span>
                  ) : (
                    '일반 계정'
                  )}
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            {recentReservations.length > 0 ? (
              ...
              <div className="bg-gray-50 rounded-lg p-12 text-center">
                ...
                <p className="text-gray-500 font-medium text-lg">예약 기록이 없습니다</p>
                <p className="text-gray-400 text-sm mt-2">아직 예약한 서비스가 없습니다</p>
              </div>
            )}
          </div>
        </div>
