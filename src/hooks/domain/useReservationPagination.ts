...
interface UseReservationPaginationResult {
  ...
  changeSort: (sortBy: PagingParams['sortBy'], sortOrder: PagingParams['sortOrder']) => Promise<void>;
  reset: () => void;
}
...
export const useReservationPagination = (): UseReservationPaginationResult => {
   ...
   * 예약 목록 조회
   */
  const fetchReservations = useCallback(async (newParams?: Partial<PagingParams>) => {
   ...
   * 상태 필터 변경
   */
  const changeStatus = useCallback(async (status?: ReservationStatus) => {
